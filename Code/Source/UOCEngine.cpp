...
void CUOCEngine::Init(HWND hWnd, int Width, int Height)
{
	...
	TwButtonCallback callback{ [](void* clientData) {
		...
		CDebugRender* l_DebugRender{ l_Engine->GetDebugRender() };
		const float l_Scale{ 1.0f };
		const XMFLOAT4 l_Color{ 1,1,1,1 };
		const XMFLOAT3 l_Position{ l_Engine->GetCameraManager()->GetCamera().GetLookAt() };
		ID3D11Device* l_Device = l_Engine->GetRenderManager()->GetDevice();
		l_DebugRender->CreateInteractiveSphere(l_DebugRender, l_PhysicsManager,l_Device, l_Scale, l_Color, l_Position);
	} };
	TwAddButton(m_Bar, "Add an Interactable Sphere", callback, NULL, "group=Physics");
...
void CUOCEngine::Render()
{
	...
	if (m_RenderManager->DrawAxisGrid())
	{
		...
		m_DebugRender->DrawInteractableSpheres(l_DeviceContext, XMFLOAT4{ 1,1,1,1 }, l_DebugRenderColor);
	}
