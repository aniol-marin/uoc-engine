...
void CFPSCameraController::Move(float Strafe, float Forward, bool Speed, float ElapsedTime)
{
	...
	l_Vector.v= XMLoadFloat3(&l_AddPos);
	l_Vector.v=DirectX::XMVector3Normalize(l_Vector);
	l_Vector.v= l_Vector*l_ConstantSpeed;
	l_Vector.v= l_Vector+DirectX::XMLoadFloat3(&m_Position);
	m_Position=XMFLOAT3(l_Vector.f[0], l_Vector.f[1], l_Vector.f[2]);
}
