...
{
public:
	virtual bool Draw(ID3D11DeviceContext *Device, CEffect *Effect) 
	{
		assert(!"This method mustn't be called");
		return false;
	}
	virtual bool DrawIndexed(ID3D11DeviceContext *Device, CEffect *Effect, unsigned int IndexCount=-1, unsigned int StartIndexLocation=0, unsigned int BaseVertexLocation=0)
	{
		assert(!"This method mustn't be called");
		return false;
	}
...
public:
	...
	bool Draw(ID3D11DeviceContext *Device, CEffect *Effect)
	{
...
public: \
	...
	: CTemplatedRenderableVertexs<T>(Device, Vtxs, VtxsCount, TopologyType, PrimitiveCount) \
	{ \
	} \
};
...
public:
	...
	bool DrawIndexed(ID3D11DeviceContext *Device, CEffect *Effect, unsigned int IndexCount=-1, unsigned int StartIndexLocation=0, unsigned int BaseVertexLocation=0)
	{
		if(Effect->GetPixelShader()==NULL || Effect->GetVertexShader()==NULL)
			return false;
...
public: \
	...
	: CTemplatedRenderableIndexedVertexs<T>(Device, Vtxs, VtxsCount, Indices, IndexsCount, TopologyType, IndexType) \
	{ \
	} \
};
